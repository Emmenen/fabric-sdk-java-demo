...
// 需要管理员身份
        ...
        System.out.println("The current chain height is " + height);
        BlockInfo blockInfo = channel.queryBlockByNumber(height-1);

        Collection<Peer> peers = channel.getPeers();
        peers.stream().forEach(peer1 -> {
            peer1.setPeerEventingServiceDisconnected(new Peer.PeerEventingServiceDisconnected() {
                @Override
                public void disconnected(Peer.PeerEventingServiceDisconnectEvent event) {
                    System.out.println(peer1.getName()+" is disconnected");
                }
            });
        });
    ...
    private static CompletableFuture<BlockEvent.TransactionEvent> sendTransaction(HFClient client, Channel channel) throws ProposalException, InvalidArgumentException {
        ...
        tpr.setArgs("CAR101", "SKoda", "MB1000", "Yellow", "Lukas");
